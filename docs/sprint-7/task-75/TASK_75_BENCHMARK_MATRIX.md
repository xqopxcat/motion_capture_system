...
Pass/fail uses the canonical classifier and target direction. Nonfinite results are `unavailable`, never pass. Report intentional interval skips separately. Viewer and Compare repeat the recorded-sync matrix when integrated; Task 75 implements neither.

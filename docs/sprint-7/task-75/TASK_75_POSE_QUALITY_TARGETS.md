...
| ID | Target / warning / failure | Unit, aggregation | Scenario | Status |
| --- | --- | --- | --- | --- |
| jitter-2d | 0.012 / 0.020 / 0.030 | normalized coordinate, landmark RMS then P95 | static matrix | Physical validation required |
| inference-latency | 50 / 80 / 120 | ms, P95 | motion/delay | Physical validation required |
| publication-latency | 80 / 120 / 200 | ms, P95 | arm/replacement | Physical validation required |
| pose-age | 120 / 200 / 300 | ms, P95 | motion | Physical validation required |
| stale-timeout | 300 / 300 / 300 | ms, configured boundary | delay/tab restore | **Confirmed existing contract** |
| low-confidence-ratio | 0.05 / 0.15 / 0.30 | ratio, frame then mean/P95 | static/low light | Physical validation required |
| unavailable-frame-ratio | 0.02 / 0.05 / 0.10 | ratio, sequence | flexion/occlusion | Physical validation required |
| sync-error | 34 / 67 / 100 | ms, P95 | review playback/seeks | Physical validation required |
| publication-rate | 20 / 15 / 10 | Hz, sequence (higher is better) | motion | Physical validation required |
| resource-growth | 5 / 10 / 20 | percent after 30 min | long session | Physical validation required |

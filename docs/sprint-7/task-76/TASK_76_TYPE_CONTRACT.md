...
`FilteredRuntimePose` is a different readonly nominal type. It preserves Raw source identity, carries the current runtime profile ID and minimal runtime-only landmark quality metadata, and is constructed solely by the runtime quality boundary. Task 77 replaced the initial identity profile with `runtime-visualization.stabilized.v1`; persistence prohibitions are unchanged.

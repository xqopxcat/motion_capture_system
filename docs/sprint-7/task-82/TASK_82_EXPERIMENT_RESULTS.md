...
通過項目：9 個 focused validation 覆蓋版本拒絕、初始化順序、固定拓樸、metric 選擇、reset/dispose、錯誤恢復、舊 session 抑制、latest-frame-wins、有限統計。序列化 harness 明確分離 2 次 warm-up 與 5 次 CI 樣本，提供 clone/JSON reference 的 min/median/mean/p95/max 與 JSON payload bytes；正常 CI 不設牆鐘門檻、不輸出逐幀資料。現行 Task 77 既有合成量測顯示 stabilization 為次毫秒級工作；Task 79 僅計算目前選取的兩個角度。

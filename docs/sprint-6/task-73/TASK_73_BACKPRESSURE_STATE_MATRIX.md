...
| Busy+Pending | frame A | frame B | candidate C | replace B with C | B coalesced | Busy+Pending | candidate, coalesced, replacement |
...
| Any active | optional | optional | pause/session rotation | clear pending; invalidate active generation | pending coalesced | Paused/Busy-obsolete | pause, coalesced; later stale rejected |
| Any | optional | optional | dispose | cancel producer, clear pending, invalidate active | pending coalesced | Disposed | coalesced; later stale rejected |

...
| --- | --- | --- | --- | --- | --- | --- |
| Local development | PostgreSQL 16 by default; migrations required | Explicit local adapter/emulator **or** dedicated private dev bucket; selection required | Explicit development provider allowed; real Google may also be used | Empty by default; developer-created real flow data only | Dev auth/storage adapter allowed only under an explicit `local` profile; in-memory repositories are not the normal runtime | Localhost CORS only; cookie may be non-Secure only on HTTP localhost; print selected adapter names; no silent fallback |
| Automated test | Isolated PostgreSQL database/schema per run for repository/integration tests; pure unit tests may inject in-memory repositories | Deterministic fake adapter or emulator with per-test namespace | Test identity/session provider; no Google network call | Factories and integration fixtures only; deterministic clocks/IDs where necessary | In-memory repositories and fake signed URLs allowed only when injected by tests | Cleanup by rollback or schema/database teardown; parallel-safe namespaces; never read developer/production credentials |
| Production-like local | PostgreSQL 16 with real migrations | Storage emulator with production adapter semantics or dedicated private dev bucket | Real Google OAuth preferred; isolated dev credentials permitted, mock endpoint disabled | Empty; created through application flow | No in-memory repository, `mock-storage.local`, Viewer fixture fallback, or mock-login | HTTPS/reverse-proxy behavior, Secure/HttpOnly/SameSite cookie, explicit CORS and secrets; same fail-fast validation as production |
| Production | Managed PostgreSQL-compatible service at approved version | Private GCS bucket or approved Task 62 equivalent | Real Google OAuth plus durable server session | No seed | All runtime mocks and development providers rejected | HTTPS-only; Secure+HttpOnly cookie; explicit SameSite/domain/path; strict CORS; secret manager/env credentials; migration/config validation before serving traffic |
...
- Storage adapter, bucket/project/emulator endpoint, signing identity/credentials, upload/download expiry, size limits, and checksum policy.
...
- storage adapter is mock/local, bucket/signing configuration is missing, or the bucket is configured public;
...
- There is no Dashboard demo seed, manual QA fake dataset, production seed, or UI data-generation button.
- A developer may create data through Capture or test factories outside the application runtime.
...
A documented command set must start frontend, backend, migrated PostgreSQL, and the chosen storage dependency under production configuration rules. Mock login and local Viewer fallback remain disabled. The environment must support the Task 65 happy path, restart test, cross-user test, signed-URL expiry test, and clean database bootstrap before production release.
